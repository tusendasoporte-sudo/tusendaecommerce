...
final class AdminApkVerifier {
    private AdminApkVerifier() {}

    static void verify(Context context, File apk, String expectedSha256, long expectedVersionCode, String expectedPackage)
            throws IOException, PackageManager.NameNotFoundException {
        if (!AdminUpdateContract.validMetadata(expectedSha256, expectedVersionCode, expectedPackage)) {
            throw new IOException("invalid_update_metadata");
        }
        if (!expectedSha256.trim().toLowerCase(Locale.ROOT).equals(sha256(apk))) {
        ...
        if (!signerSet(archive).equals(signerSet(installed)) || signerSet(archive).isEmpty()) {
    ...
    @SuppressWarnings("deprecation")
    private static PackageInfo installedInfo(PackageManager manager, String packageName) throws PackageManager.NameNotFoundException {
        int flags = Build.VERSION.SDK_INT >= 28 ? PackageManager.GET_SIGNING_CERTIFICATES : PackageManager.GET_SIGNATURES;
    ...
    private static Set<String> signerSet(PackageInfo info) {
        ...
        if (signatures != null) Arrays.stream(signatures).forEach((signature) -> result.add(Arrays.toString(signature.toByteArray())));
        return result;
    }
}
