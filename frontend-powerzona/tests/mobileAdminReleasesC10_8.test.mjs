...
test('portal y proxy conservan sesión, dispositivo y APK privada sin asignación individual', () => {
  ...
  assert.match(portal, /<script is:inline>/);
  assert.match(portal, /<style is:inline>/);
  assert.match(portal, /searchParams\.set\('handoff', 'legacy-webview'\)/);
  assert.match(portal, /toca la notificación para instalar/i);
  ...
  assert.match(downloadApi, /parseNativeAdminAppUserAgent/);
  assert.match(downloadApi, /sec-fetch-mode/);
  assert.match(downloadApi, /X-PZ-Download-Handoff/);
  assert.ok(downloadApi.indexOf('isLegacyWebViewHandoff(request)') < downloadApi.indexOf('const baseUrl'));
...
test('enforcement afecta la app antigua y deja navegadores normales fuera del gate', () => {
  ...
  assert.match(updatePrompt, /<script is:inline>/);
  assert.match(updatePrompt, /<style is:inline>/);
  assert.match(updatePrompt, /searchParams\.set\('handoff', 'legacy-webview'\)/);
  assert.match(updatePrompt, /toca la notificación para instalar/i);
