...
test('SERVICIO renderiza navegación inmersiva en la misma pestaña, galerías y cotización accesible', () => {
  ...
  assert.match(detail, /labels\.previousPhoto/);
  assert.match(detail, /labels\.nextPhoto/);
  ...
  assert.match(styles, /min-height: clamp\(19rem, 36svh, 24rem\)/);
  assert.match(styles, /\.promo-service-detail__arrow--previous/);
  assert.match(styles, /\.promo-service-detail__arrow--next/);
