...
function normalizeProfile(value: any): AdminAppProfile | null {
  ...
  return {
    ...
    latest_version_name: text(value.latest_version_name, 40), minimum_supported_version_code: integer(value.minimum_supported_version_code),
    status: value.status,
  };
}
...
function normalizeJob(value: any): AdminAppJob | null {
  ...
  return {
    ...
    preview_expires_at: text(value.preview_expires_at, 80), failure_code: text(value.failure_code, 80),
  };
}
...
export function getMasterAdminAppDetail(baseUrl: string, token: string, channel: 'staging' | 'production') {
  return post(baseUrl, token, '/api/pz/master/admin-app-releases/detail', { channel }, (value) => {
    ...
    if ((value.profile && !profile) || jobs.length !== (value.jobs || []).length || artifacts.length !== (value.artifacts || []).length
    ...
    return { profile, jobs, artifacts, assignments, eligible_devices, events: Array.isArray(value.events) ? value.events : [] } as MasterAdminAppDetail;
  });
}
...
export function previewMasterAdminAppBuild(baseUrl: string, token: string, input: { channel: string; version_code: number; version_name: string }) {
