...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  if (!baseUrl) {
    return json({ ok: false, message: 'No se pudo validar tu participación. Intenta nuevamente.' }, 503);
  }

  try {
    const response = await fetch(`${baseUrl}/api/pz/raffles/status`, {
      method: 'POST',
    ...
    if (!result || typeof result !== 'object') {
      return json({ ok: false, message: 'No se pudo validar tu participación. Intenta nuevamente.' }, 502);
    }
