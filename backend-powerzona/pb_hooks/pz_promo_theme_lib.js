...
const BLACK_GOLD_MANIFEST_SHA256 = "dfd9455fb313cac15c1a2b74dbf43dda7d101e3ad8d7aa2a8f6a0306f593fde8";
...
function assertApprovedCombination(tokens) {
  if (tokens.accent !== tokens.border) fail("incompatible_promo_theme_tokens", 400);
  if (tokens.surface !== "obsidian" || tokens.text !== "ivory" || tokens.focus !== "ivory_ring") {
...
function assertAccessibleCombination(tokens) {
  ...
  if (!surface || !text || !accent || !focus
    || contrastRatio(surface, text) < BLACK_GOLD_MANIFEST.accessibility.normal_text_contrast_min
    || contrastRatio(surface, accent) < BLACK_GOLD_MANIFEST.accessibility.large_text_contrast_min
    || contrastRatio(surface, focus) < BLACK_GOLD_MANIFEST.accessibility.focus_contrast_min) {
...
function validateOverrides(entry, input) {
  ...
  assertApprovedCombination(effective);
  assertAccessibleCombination(effective);
