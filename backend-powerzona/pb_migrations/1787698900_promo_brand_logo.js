...
migrate((app) => {
  ...
  if (!purpose.values.includes(LOGO_MEDIA_PURPOSE)) purpose.values.push(LOGO_MEDIA_PURPOSE);
  return app.save(media);
}, (app) => {
  const used = app.findRecordsByFilter(
    "promo_media_assets", "purpose = {:purpose}", "id", 1, 0, { purpose: LOGO_MEDIA_PURPOSE },
  ) || [];
  if (used.length) throw new Error("unsafe_rollback_promo_logo_media");
  const { media, purpose } = purposeField(app);
  purpose.values = purpose.values.filter((value) => value !== LOGO_MEDIA_PURPOSE);
  return app.save(media);
});
