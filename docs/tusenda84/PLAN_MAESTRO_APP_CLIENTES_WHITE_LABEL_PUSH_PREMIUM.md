...
- `mobile-admin` es una aplicación Android nativa independiente, actualmente `1.0.2 (3)`, con paquete predeterminado `com.tusenda84.admin`, Java 17, minSdk 26 y targetSdk 36. Su WebView conserva la sesión del panel, restringe hosts, bloquea HTTP en release y cancela errores SSL.
- El acceso de tienda ya exige usuario activo y el encabezado/cookie `X-PZ-Admin-Device` / `pz_admin_device`. PocketBase guarda únicamente el digest SHA-256 en `store_user_devices`, comprueba el dispositivo al iniciar y refrescar sesión y permite revocarlo con auditoría y cierre de sesiones.
- El motor C10.7 ya aporta trabajos idempotentes, continuidad de paquete/firma/versión, carga física verificada por SHA-256 y archivos protegidos dentro de `pb_data`. Su enlace HMAC anónimo y compartible no es adecuado para la aplicación administrativa.
...
|---|---|
| `admin_app_release_profiles` | Identidad Android, configuración editable, recursos visuales activos, último código reservado/publicado y versión mínima permitida. |
...
### 2026-08-18 — PZ-APP-C10.8 DISEÑADO: Mobile Admin protegido

- El propietario definió cuatro condiciones: descarga solo después de autenticarse, enlace asociado al administrador, piloto inicial en un dispositivo autorizado, publicación gradual y actualización obligatoria únicamente después de validar.
- La auditoría confirmó que `mobile-admin` está en `1.0.2 (3)` con paquete `com.tusenda84.admin`, firma release externa al repositorio, WebView HTTPS y registro administrativo existente en `store_user_devices`. También confirmó que el enlace HMAC compartible de C10.7 no puede reutilizarse para esta entrega más sensible.
- Se diseñó un dominio administrativo separado que comparte runner, transferencia, checksum y custodia física, pero exige sesión, usuario, tienda, dispositivo, asignación y ticket corto de un uso en cada descarga. El grant visible se almacenará únicamente como hash y dejará de funcionar al revocar cualquiera de esas relaciones.
- El rollout queda fijado como `draft` → `pilot` de un administrador/dispositivo → oleadas con destinatarios explícitos → `general` → versión mínima obligatoria. Descargar no valida el piloto; se exige instalación, login, check-in y aprobación Master.
- La obligatoriedad queda limitada a sesiones nativas de `mobile-admin`; los navegadores administrativos siguen disponibles. El update conserva rutas de login, logout, recuperación y descarga para evitar un bloqueo irrecuperable, y puede desactivarse server-side. Un APK defectuoso ya instalado se recupera con hotfix de `versionCode` superior, no con downgrade Android.
- En esta fase solo se inspeccionó y documentó. No se modificó código operativo, no se generaron APK, claves o firmas, no se tocó Firebase, no se desplegó, no se instaló en dispositivos y no se enviaron enlaces o mensajes.

### 2026-08-18 — PZ-APP-C10.8 IMPLEMENTADO LOCALMENTE: entrega y rollout de Mobile Admin

- Se añadieron seis colecciones privadas `admin_app_*`: perfil de identidad, trabajos idempotentes, artefactos APK protegidos, asignaciones exactas, tickets efímeros de descarga y auditoría. El borrado de tienda o administrador limpia o desacopla únicamente sus relaciones administrativas sin afectar artefactos compartidos.
- El backend aplica continuidad de paquete, huella de certificado y `versionCode`; grant almacenado solo como digest; ticket de dos minutos y un uso; sesión, tienda, administrador y dispositivo autorizado exactos; piloto individual, oleadas explícitas, publicación general, pausa/retiro y versión mínima solo después de validar.
- El panel Master puede preparar trabajos, confirmar el contrato, administrar piloto/oleadas y copiar el enlace autenticado. El portal del administrador solicita el ticket sin exponer el APK y el middleware limita el enforcement a sesiones identificadas de la app nativa, dejando navegadores normales sin bloqueo.
- `mobile-admin` incorpora puente nativo de check-in y descarga, limpieza de APK anteriores en caché, verificación de SHA-256, paquete, `versionCode` superior y certificado antes de abrir el instalador. El runner y `scripts/build-admin-app.ps1` exigen configuración de firma existente; no crean claves, Firebase ni credenciales.
- La migración y el flujo completo se probaron con PocketBase real en un directorio temporal: creación, asignación, ticket, descarga, check-in, validación, publicación, obligatoriedad, revocación y limpieza. Las pruebas focales backend terminaron 15/15, frontend 8/8, runtime C10.8 1/1 y Android `testDebugUnitTest`, `lintDebug` y `assembleDebug` correctamente.
- La regresión frontend completa terminó 542/542. La repetición final de la regresión backend completa terminó con 719 pruebas totales, 712 aprobadas, 7 omitidas y 0 fallos; la ejecución concurrente anterior que había expuesto una inestabilidad histórica quedó así descartada sin cambios ajenos a C10.8.
- No se utilizó firma real, no se compiló release de producción, no se tocó Firebase, no se desplegó, no se instaló en dispositivos, no se envió WhatsApp ni enlace y no se publicó en Google Play. La validación en staging y el piloto físico permanecen pendientes de autorización separada.

### 2026-08-18 — PZ-APP-C10.8 IMPLEMENTADO LOCALMENTE: Tu Senda 84 Admin Engine 1.0.0 y panel simplificado

- El propietario aclaró que PowerZona es una tienda dentro de Tu Senda 84. El constructor queda nombrado `Tu Senda 84 Admin Engine 1.0.0` y Mobile Admin se mantiene genérico para ecommerce, páginas promocionales, servicios y futuros tipos de proyecto que compartan el acceso administrativo central.
- El panel muestra el motor de solo lectura, elimina la entrada manual de `versionCode` y presenta el siguiente código calculado por el servidor. El baseline `0` representa una app nueva; el primer build recibe `1`. La confirmación reserva el número transaccionalmente y los fallos no lo liberan.
- Identidad Android y configuración editable quedaron separadas. Paquete, firma y canal se protegen al confirmar el primer build; antes de ese punto también se puede corregir la versión instalada declarada. Nombre, URL, Firebase, color, icono y splash se pueden actualizar, pero cada vista previa conserva una fotografía exacta y se invalida si cambia.
- Se añadió custodia privada y versionada para icono y splash, descarga exclusiva de Master/runner, comprobación PNG y SHA-256, aplicación temporal al proyecto Android y restauración segura. Android 8–11 usa el fondo de lanzamiento generado y Android 12+ sus atributos nativos con el mismo color y recurso.
- La identificación nativa incluye el `applicationId` real, además de versión y código. Así la APK de staging `com.tusenda84.admin.staging` consulta su propio perfil y nunca se confunde con `com.tusenda84.admin`; las versiones C10.8 anteriores conservan compatibilidad cerrada con producción.
- La interfaz ya no pide etapa ni oleada. Expone las acciones de negocio “Enviar al dispositivo de prueba”, “Aprobar prueba”, “Añadir administrador”, “Publicar para todos” y “Solicitar última versión”; el backend conserva la progresión y auditoría internas.
- La migración reconoce trabajos preexistentes como motor histórico `0.0.0`, invalida vistas previas antiguas, detiene trabajos antiguos todavía activos y conserva como consumido cualquier código previamente confirmado.
...
- Tras desplegar `38e6a20` en ambos servicios de staging, frontend y backend respondieron `200`. Siete muestras estables de portada registraron medianas internas de 8,9 ms en seguridad, 5,1 ms en SSR/datos y 15,0 ms totales, frente a TTFB externo de 735 ms. Una primera muestra aislada alcanzó 849,8 ms internos y queda como posible arranque/estado frío por repetir. No se atribuye la diferencia externa a PocketBase sin evidencia adicional.
- El paso 2 quedó implementado localmente sin despliegue: conexión anticipada al origen de medios, precarga en `<head>` de la misma primera imagen de portada, `fetchpriority=high` solo para ella, dimensiones 1600x900 y decodificación asíncrona. Se conservan archivo WebP original, calidad, recorte, `object-fit: cover`, carrusel y carga diferida del resto. Pasaron `48/48` pruebas focales, build Astro SSR y `git diff --check`.
- Con `e4fcf94` desplegado en staging, seis cargas Edge confirmaron que la portada pasa a ser la solicitud número 2 y siempre prioridad `High`; producción aún la descubre como número 7. Las medianas staging/producción fueron LCP 1.408/2.708 ms en escritorio y 1.256/2.168 ms en móvil, con CLS 0. La comparación es favorable pero no atribuye todo al código: la portada staging pesa unos 54 KB frente a 159 KB, mientras su página completa pesa aproximadamente 1,49 MB frente a 1,05 MB. La captura visual fue correcta y se verificaron cero registros nuevos de analítica/seguridad en la ventana; todos los artefactos temporales fueron eliminados.
- La auditoría 3A de C14 midió `1.451.810` bytes en las 10 imágenes únicas de PocketBase de la portada staging. Se aisló una segunda conversión perjudicial: Regalos pasaba de un WebP almacenado de `78.222` bytes y `1200x675` a un PNG de `323.948` bytes y `700x420`. Con autorización se cambió localmente solo esa URL para entregar el WebP ya optimizado; el ahorro previsto es `245.726` bytes, cerca del 17 % del peso de imágenes. Portada, categorías, productos, elementos visuales, datos e infraestructura quedaron intactos. Pasaron `11/11` pruebas focales, build Astro SSR y `git diff --check`.
- Tras desplegar `644c780`, Regalos confirmó el banner WebP de `78.222` bytes y sus dos fotos JPEG individuales conservaron correctamente miniaturas que ahorran `45.741` bytes. La auditoría de ocho páginas públicas encontró tres productos WebP cuyas miniaturas PNG pesan `209.699` bytes frente a `150.008` bytes de los originales. Con autorización se implementó localmente una regla limitada a tarjetas de productos: WebP directo y miniatura para formatos heredados. La simulación en seis combinaciones escritorio/móvil conservó dimensiones y composición sin imágenes rotas ni desbordamiento. Pasaron `15/15` pruebas focales, build Astro SSR y `git diff --check`; no se modificaron datos, imágenes, Regalos, detalles ni variaciones.
- La auditoría 3C comprobó que `480x270` no reduce los tres JPEG/PNG heredados de categorías/subcategorías y que `300x200` recorta su composición. La simulación WebP `0.82`, manteniendo dimensiones exactas, redujo `956.876` a `155.506` bytes (`83,7 %`) sin diferencia de composición en escritorio o móvil. Con autorización se implementó localmente una regla única para los tres flujos de carga: solo convierte JPEG/PNG, conserva ancho/alto y usa el WebP únicamente si pesa menos; WebP existente, productos, variaciones y Regalos no cambian. Pasaron `43/43` pruebas focales y de regresión y el build Astro SSR. Las imágenes actuales siguen intactas; la carga aislada y revisión visual en staging quedan pendientes de despliegue autorizado y limpieza total posterior.
