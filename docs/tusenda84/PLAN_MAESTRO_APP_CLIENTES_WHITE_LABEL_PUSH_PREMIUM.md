...
- Migración append-only local `1786665600_push_campaign_retention_7d.js`: añade a `stores` el JSON oculto `push_campaign_quota_state`, retroactiva `delete_after` de campañas existentes y conserva solo ID técnico/fecha de inicio/zona horaria necesarios para 10/310. No se hará push, despliegue, reinicio, parada ni cambio externo sin autorización explícita separada.
...
- Estimación automática solicitada durante C08: al abrir una campaña guardada, el panel consulta inmediatamente a C05 y muestra cuántas instalaciones elegibles alcanzaría. El cálculo anuncia carga/resultado mediante `aria-live`, distingue estimación dinámica de `snapshot`, evita que una respuesta tardía sobrescriba otra campaña y conserva `Actualizar cálculo` para cambios editables. La apertura no guarda el borrador, no inicia envíos ni consume cuota.
...
- El ajuste local de siete días modifica C05 solo en retención/scheduler y añade una migración de PocketBase; C04, Firebase, App Check, Cloudflare, firma Android, `google-services.json`, relay storefront v2 y relay administrativo v1 no cambian. No se ha hecho push ni despliegue de este ajuste.
...
| Audiencia estimada | [x] | [ ] | Al abrir una campaña guardada, el conteo se calcula automáticamente; `snapshot` y estimación dinámica se etiquetan correctamente. `Actualizar cálculo` refresca cambios editables. |
...
- Mantener exclusivamente C08 `EN CURSO`. El ajuste de retención de siete días está implementado y validado solo en local; no se ha publicado. Antes de hacerlo se requiere autorización explícita separada para push y para redesplegar tanto frontend staging como PocketBase staging, con aviso previo del impacto y prueba manual antes/después. Luego comprobar la fila manual de retención, completar los casos C08 aún abiertos y cerrar solo C08. No iniciar C09.
